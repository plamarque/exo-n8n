...
const gPT4oMiniModel = languageModel({
  ...
  version: 1.3,
  config: {name: "GPT-4o Mini Model", parameters: {model: {__rl: true, mode: "list", value: "gpt-4o-mini"}, builtInTools: {}, options: {temperature: 0.3}}, position: [4280, 800]}
});

const structuredOutput = outputParser({
  type: "@n8n/n8n-nodes-langchain.outputParserStructured",
  version: 1.3,
  config: {name: "Structured Output", parameters: {jsonSchemaExample: "{\"description\":\"A concise 30-word description\",\"suggestedCategories\":[\"Category1\",\"Category2\"]}"}, position: [4408, 800]}
});

const manualStart = trigger({
  type: "n8n-nodes-base.manualTrigger",
  version: 1,
  config: {name: "Manual Start", parameters: {}, position: [240, 288]},
  output: [{}]
});
...
const workflowInput = node({
  ...
  version: 3.4,
  config: {name: "Workflow Input", parameters: {assignments: {assignments: [{id: "1", name: "spaceName", value: expr("{{ $vars.EXO_SPACE_NAME }}"), type: "string"}]}, options: {}}, position: [464, 384]},
...
const iFSpaceName = ifElse({
  ...
  version: 2.3,
  config: {name: "IF Space Name", parameters: {conditions: {options: {caseSensitive: true, leftValue: "", typeValidation: "strict"}, conditions: [{id: "s0", leftValue: expr("{{ String($json.spaceName || '').trim() }}"), rightValue: "", operator: {type: "string", operation: "notEmpty"}}], combinator: "and"}, options: {}}, position: [688, 384]},
  output: [{}]
});
...
const ensureTrackingTable = node({
  ...
  version: 1.1,
  config: {name: "Ensure Tracking Table", parameters: {resource: "table", operation: "create", tableName: "exo_processed_documents", columns: {column: [{name: "documentId"}, {name: "lastProcessedDate", type: "date"}, {name: "description"}, {name: "categories"}, {name: "spaceName"}, {name: "documentName"}, {name: "documentUrl"}, {name: "editorUrl"}]}, options: {createIfNotExists: true}}, position: [912, 480], onError: "continueRegularOutput"},
  output: [{}]
});
...
const getSpaces = node({
  ...
  version: 1,
  config: {name: "Get Spaces", parameters: {endpointUrl: expr("{{$vars.EXO_MCP_ENDPOINT}}"), authentication: "mcpOAuth2Api", tool: {__rl: true, value: "get_my_spaces", mode: "list", cachedResultName: "get_my_spaces"}, inputMode: "json", jsonInput: {}, options: {timeout: 120000}}, position: [1136, 480]},
...
const resolveSpace = node({
  type: "n8n-nodes-base.set",
  version: 3.4,
  config: {name: "Resolve Space", parameters: {assignments: {assignments: [{id: "1", name: "spaceName", value: expr("{{ String($('Workflow Input').item.json.spaceName || '').trim() }}"), type: "string"}, {id: "2", name: "spaceId", value: expr("{{ ($json.content?.[0]?.text || []).find((s) => s.name === String($('Workflow Input').item.json.spaceName || '').trim())?.space_id }}"), type: "number"}]}, options: {}}, position: [1360, 480]},
...
const iFSpaceResolved = ifElse({
  type: "n8n-nodes-base.if",
  version: 2.3,
  config: {name: "IF Space Resolved", parameters: {conditions: {options: {caseSensitive: true, leftValue: "", typeValidation: "strict", version: 3}, conditions: [{id: "k0", leftValue: expr("{{ $json.spaceId != null && $json.spaceId !== \"\" && !Number.isNaN(Number($json.spaceId)) }}"), rightValue: true, operator: {type: "boolean", operation: "true", singleValue: true}}], combinator: "and"}, options: {}}, position: [1584, 480]},
  output: [{}]
});
...
const splitOutDocuments = node({
  ...
  version: 1,
  config: {name: "Split Out Documents", parameters: {fieldToSplitOut: "content[0].text", options: {}}, position: [2032, 672]},
...
const filterHasDocumentId = node({
  ...
  version: 2.3,
  config: {name: "Filter - Has document_id", parameters: {conditions: {options: {caseSensitive: true, leftValue: "", typeValidation: "strict", version: 3}, conditions: [{id: "d0", leftValue: expr("{{ String($json.document_id || '') }}"), rightValue: "", operator: {type: "string", operation: "notEmpty"}}], combinator: "and"}, options: {}}, position: [2256, 672]},
  output: [{document_id: "doc1", updated_date: "2026-04-24", description: ""}]
});
...
const normalizeDocuments = node({
  type: "n8n-nodes-base.set",
  version: 3.4,
  config: {name: "Normalize Documents", parameters: {assignments: {assignments: [{id: "1", name: "id", value: expr("{{ String($json.document_id) }}"), type: "string"}, {id: "2", name: "updatedDate", value: expr("{{ $json.updated_date }}"), type: "string"}, {id: "3", name: "description", value: expr("{{ $json.description ?? '' }}"), type: "string"}]}, options: {}}, position: [2480, 672]},
...
const mergeDocumentsToProcess = merge({
  ...
  version: 3.2,
  config: {name: "Merge Documents to Process", parameters: {mode: "combineBySql", query: "SELECT input1.id, input1.updatedDate, input1.description\nFROM input1\nLEFT JOIN input2 ON input1.id = input2.documentId\nWHERE input2.documentId IS NULL\n   OR input2.lastProcessedDate IS NULL\n   OR input2.lastProcessedDate = ''\n   OR input1.updatedDate > input2.lastProcessedDate", options: {emptyQueryResult: "empty"}}, position: [2928, 672]}
});

const stopSpaceNotFound = node({
  type: "n8n-nodes-base.stopAndError",
  version: 1,
  config: {name: "Stop - Space not found", parameters: {errorMessage: "Espace eXo introuvable (aucun espace de ce nom pour l’utilisateur)."}, position: [1808, 480]},
  output: [{}]
});
...
const stopMissingSpaceName = node({
  type: "n8n-nodes-base.stopAndError",
  version: 1,
  config: {name: "Stop - Missing spaceName", parameters: {errorMessage: "Missing required workflow input: 'spaceName'"}, position: [912, 288]},
  output: [{}]
});
...
const getProcessedForDoc = node({
  type: "n8n-nodes-base.dataTable",
  version: 1.1,
  config: {name: "Get Processed For Doc", parameters: {operation: "get", dataTableId: {__rl: true, mode: "name", value: "exo_processed_documents"}, returnAll: true}, position: [2704, 772], executeOnce: true, alwaysOutputData: true, onError: "continueRegularOutput"},
  output: [{}]
});
...
const limitTo5Documents = node({
  ...
  version: 1,
  config: {name: "Limit to 5 Documents", parameters: {maxItems: 5}, position: [3152, 672]},
  output: [{}]
});
...
const processingSummary = node({
  type: "n8n-nodes-base.set",
  version: 3.4,
  config: {name: "Processing Summary", parameters: {assignments: {assignments: [{id: "1", name: "message", value: "Document enrichment completed successfully", type: "string"}, {id: "2", name: "processedCount", value: expr("{{ $(\"Update Tracking\").all().length }}"), type: "number"}, {id: "3", name: "timestamp", value: expr("{{ $now.toISO() }}"), type: "string"}]}, options: {}}, position: [3600, 384]},
  output: [{}]
});
...
const readDocumentContent = node({
  type: "@n8n/n8n-nodes-langchain.mcpClient",
  version: 1,
  config: {name: "Read Document Content", parameters: {endpointUrl: expr("{{$vars.EXO_MCP_ENDPOINT}}"), authentication: "mcpOAuth2Api", tool: {__rl: true, mode: "list", value: "get_document_by_id", cachedResultName: "get_document_by_id"}, inputMode: "json", jsonInput: expr("{{ { \"document_id\": $json.id } }}"), options: {timeout: 120000}}, position: [3600, 576]},
...
const listCategories = node({
  type: "@n8n/n8n-nodes-langchain.mcpClient",
  version: 1,
  config: {name: "List Categories", parameters: {endpointUrl: expr("{{$vars.EXO_MCP_ENDPOINT}}"), authentication: "mcpOAuth2Api", tool: {__rl: true, value: "get_category_tree", mode: "list", cachedResultName: "get_category_tree"}, inputMode: "json", jsonInput: {}, options: {timeout: 120000}}, position: [3824, 576]},
...
const prepareCategoryAssignments = node({
  ...
  version: 2,
  config: {name: "Prepare Category Assignments", parameters: {jsCode: "const normalize = (s) => String(s || '').replace(/&amp;/g, '&').replace(/\\s+/g, ' ').trim().toLowerCase();\nconst tree = $(\"List Categories\").item.json.content?.[0]?.text;\nconst map = new Map();\nconst walk = (node) => {\n  if (!node) return;\n  if (Array.isArray(node)) return node.forEach(walk);\n  if (node.name && node.category_id != null) map.set(normalize(node.name), node.category_id);\n  if (Array.isArray(node.sub_categories)) walk(node.sub_categories);\n};\nwalk(tree?.sub_categories || tree);\nconst id = map.get(normalize($json.categoryName));\nif (id == null) throw new Error('No category_id match for suggested category: ' + String($json.categoryName));\nreturn [{ json: { ...$json, category_id: id, content_id: $json.documentId, content_type: 'document' } }];"}, position: [5744, 672]},
  output: [{documentId: "doc1", categoryName: "Category1", category_id: 1, content_id: "doc1", content_type: "document"}]
});

const assignCategories = node({
  type: "@n8n/n8n-nodes-langchain.mcpClient",
  version: 1,
  config: {name: "Assign Categories", parameters: {endpointUrl: expr("{{$vars.EXO_MCP_ENDPOINT}}"), authentication: "mcpOAuth2Api", tool: {__rl: true, value: "add_content_to_category", mode: "list", cachedResultName: "add_content_to_category"}, inputMode: "json", jsonInput: expr("{{ { \"category_id\": $json.category_id, \"content_id\": $json.content_id, \"content_type\": $json.content_type } }}"), options: {timeout: 120000}}, position: [5744, 672]},
  output: [{}]
});
...
const iFAssignMCPOK = ifElse({
  type: "n8n-nodes-base.if",
  version: 2.3,
  config: {name: "IF Assign MCP OK", parameters: {conditions: {options: {caseSensitive: true, leftValue: "", typeValidation: "strict", version: 3}, conditions: [{id: "b0", leftValue: expr("{{ !String($json.content?.[0]?.text ?? '').toLowerCase().includes('exception') }}"), rightValue: true, operator: {type: "boolean", operation: "true", singleValue: true}}], combinator: "and"}, options: {}}, position: [5968, 672]},
  output: [{}]
});
...
const checkAssignResult = node({
  ...
  version: 1,
  config: {name: "Check Assign Result", parameters: {}, position: [6192, 768]},
  output: [{}]
});
...
const processEachDocument = splitInBatches({
  ...
  version: 3,
  config: {name: "Process Each Document", parameters: {options: {}}, position: [3376, 672]},
  output: [{}]
});
...
const stopCategoryAssignFailed = node({
  type: "n8n-nodes-base.stopAndError",
  version: 1,
  config: {name: "Stop - Category assign failed", parameters: {errorMessage: "MCP add_content_to_category: échec."}, position: [6192, 576]},
  output: [{}]
});
...
const stopDescriptionUpdateFailed = node({
  type: "n8n-nodes-base.stopAndError",
  version: 1,
  config: {name: "Stop - Description update failed", parameters: {errorMessage: "MCP update_document_description: réponse d’échec."}, position: [5296, 480]},
  output: [{}]
});
...
const dailySchedule = trigger({
  ...
  version: 1.3,
  config: {name: "Daily Schedule", parameters: {rule: {interval: [{triggerAtHour: 2}]}}, position: [240, 480]},
  output: [{}]
});

const iFSuggestedCategories = ifElse({
  type: "n8n-nodes-base.if",
  version: 2.3,
  config: {name: "IF Suggested Categories", parameters: {conditions: {options: {caseSensitive: true, leftValue: "", typeValidation: "strict", version: 3}, conditions: [{id: "sg0", leftValue: expr("{{ Array.isArray($json.suggestedCategories) && $json.suggestedCategories.length > 0 }}"), rightValue: true, operator: {type: "boolean", operation: "true", singleValue: true}}], combinator: "and"}, options: {}}, position: [5520, 672]},
  output: [{}]
});

const stopNoSuggestedCategories = node({
  type: "n8n-nodes-base.stopAndError",
  version: 1,
  config: {name: "Stop - No suggested categories", parameters: {errorType: "errorMessage", errorMessage: "Aucune catégorie suggérée pour le document."}, position: [5520, 544]},
  output: [{}]
});

const splitSuggestedCategories = node({
  type: "n8n-nodes-base.splitOut",
  version: 1,
  config: {name: "Split Suggested Categories", parameters: {fieldToSplitOut: "suggestedCategories", include: "allOtherFields", options: {destinationFieldName: "categoryName"}}, position: [5744, 544]},
  output: [{documentId: "doc1", categoryName: "Category1"}]
});
...
const categoryPrepFlow = iFSuggestedCategories
  .onTrue(splitSuggestedCategories.to(prepareCategoryAssignments.to(assignCategories.to(assignCheckFlow))))
  .onFalse(stopNoSuggestedCategories);

const descriptionCheckFlow = iFDescriptionMCPOK
  .onTrue(checkDescriptionResult.to(categoryPrepFlow))
