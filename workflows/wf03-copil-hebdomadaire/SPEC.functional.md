...
> Specs techniques d’exploration : [SPEC.technical-exo-mips.md](SPEC.technical-exo-mips.md) (MCP eXo MIPS) et [SPEC.technical-mcp.md](SPEC.technical-mcp.md) (MCP eXo QAUI). Modèle de note : [fixtures/copil-template-note.md](fixtures/copil-template-note.md). Synthèse portefeuille : [`../../docs/SPEC.md`](../../docs/SPEC.md).
...
- un ordre du jour suggere produit par IA a partir de l'analyse des taches
...
5. Une analyse IA du projet suggere des points de vigilance et des sujets a mettre a l'ordre du jour.
6. Une invitation agenda recurrente est creee ou maintenue sur le creneau defini.
7. La description de l'invitation contient le lien vers la note de la semaine, afin que chacun puisse la consulter en amont.
...
5. Premiere interpretation du projet par IA pour faire ressortir les vigilances utiles au pilotage.
6. Diffusion simple du bon lien au bon moment, sans manipulation manuelle.
...
- note template source: `25`
- note parente de rangement des comptes rendus generes: `6`
- projet de reference pour le rapport d'avancement global: `3`
- creneau recurrent du copil: chaque jeudi a `10:00`
...
- une zone "ordre du jour suggere" alimentee par IA
- une zone "points de vigilance"
- une zone "decisions / arbitrages"
- une zone "actions / prochains pas"

### 5.2 Un rapport tabulaire d'avancement
Le rapport est construit a partir des taches retenues dans le perimetre du projet `3`.
...
### 5.3 Une lecture IA du projet
En complement du tableau, le workflow produit une lecture interpretee du projet afin d'aider l'equipe a preparer la reunion.

Cette lecture doit rester non bloquante et non prescriptive:
- elle suggere des sujets
- elle n'impose pas les decisions
- elle laisse aux participants la responsabilite de retenir ou non les points proposes

Les suggestions IA peuvent notamment faire remonter:
- une tache urgente bloquee ou en attente depuis longtemps
- une tache qui n'a pas evolue depuis une duree significative
- une accumulation anormale de taches dans une meme colonne ou un meme statut
- un volume inhabituel de retards
- des signaux faibles de saturation, d'oubli ou de manque d'avancement sur un perimetre

Le resultat attendu est une liste courte et exploitable de points de vigilance et de sujets suggeres pour l'ordre du jour.

### 5.4 Une invitation agenda recurrente
Une invitation de reunion est positionnee chaque semaine sur le meme creneau.

Elle doit comporter:
- le titre de la reunion
- la date et l'heure du creneau, fixes le jeudi a `10:00`
...
Le scenario cible est hebdomadaire, chaque jeudi matin a `10:00`.

Exemple de logique metier attendue:
1. Le workflow s'execute avant le copil du jeudi a `10:00`, sur un horaire de preparation defini a l'avance.
...
5. Il cree cette note comme note fille de la note `6`, qui sert de conteneur des comptes rendus.
...
9. Il analyse ces taches par IA pour identifier des signaux de vigilance et proposer un ordre du jour suggere.
10. Il insere le tableau dans la section prevue de la note.
11. Il renseigne dans la note les sujets suggeres, points de vigilance et alertes de pilotage proposes par l'IA.
12. Il complete au besoin les rubriques standard deja presentes dans le template:
- ordre du jour
- avancement
- points de blocage
- decisions
- actions
13. Il met a jour l'invitation agenda recurrente si elle existe et est exploitable; sinon il recree un evenement recurrent equivalent.
14. Il injecte dans la description de l'invitation le lien vers la note de la semaine.
15. Les participants disposent alors d'une invitation a jour et d'un support de reunion deja prepare.
...
6. Les notes generees doivent etre creees comme filles de la note `6`.
7. Le template de reference de ce workflow est la note `25`.
8. Les suggestions IA doivent etre clairement presentees comme des propositions d'analyse, non comme des decisions de reunion.

### 8.2 Regles sur le rapport tabulaire
1. Le rapport ne doit presenter que les taches dans le perimetre retenu pour le copil.
Le perimetre de reference retenu pour ce workflow est le projet `3`.
...
### 8.3 Regles sur l'analyse IA
1. L'analyse IA doit etre basee sur la lecture des taches du projet `3`.
2. Elle doit produire des suggestions courtes, lisibles et directement actionnables en contexte de copil.
3. Elle peut faire remonter des signaux de vigilance, sans pretendre etablir une verite definitive sur le projet.
4. Les sujets suggeres doivent etre relies a des constats observables dans les taches.
5. Les participants restent libres de traiter ou non les points proposes pendant la reunion.
6. Seuils de vigilance retenus:
- stagnation de tache: `X=3` jours sans avancement significatif
- urgence bloquee/en attente: `Y=5` jours
- surcharge de colonne/statut: `Z=5` taches ou plus dans la meme colonne

### 8.4 Regles sur l'invitation agenda
1. La recurrence est hebdomadaire sur un creneau fixe.
Le creneau fixe retenu est le jeudi a `10:00`.
...
5. Strategie de maintien de la recurrence:
- priorite a la mise a jour de l'evenement recurrent existant
- si le tool ne permet pas de garantir/identifier la recurrence, recreation d'un evenement recurrent propre
...
## Points de vigilance suggeres
[liste generee automatiquement]
...
- un template de note de copil: note `25`
- une note parente de rangement des comptes rendus: note `6`
- un perimetre de taches a analyser: projet `3`
- un creneau hebdomadaire de reunion: chaque jeudi a `10:00`
- une liste de participants recurrente:
  `claire`, `etienne`, `louis`, `nadia`, `antoine`, `emma`
- une convention de nommage de la note
- une logique d'analyse IA produisant suggestions et vigilances a partir des taches

### 10.2 Sorties attendues
- une note de copil creee pour la semaine cible
- un tableau d'avancement insere dans cette note
- une liste de sujets suggeres par l'IA pour alimenter l'ordre du jour
- une liste de points de vigilance detectes dans les taches
...
- des points d'attention sont proposes par l'IA
...
6. Si la note parente `6` est inaccessible, le workflow ne doit pas creer la note de copil a un autre emplacement sans validation explicite.
7. Si le template `25` change de structure, l'insertion du rapport doit continuer a viser la section prevue pour l'avancement.
8. Si le projet `3` n'est pas accessible au moment de la preparation, le workflow ne doit pas produire un faux rapport d'avancement silencieux.
9. Si l'analyse IA ne detecte aucun signal saillant, la note doit quand meme etre generee avec une section de suggestions explicitement vide ou neutre.
10. Les invitations agenda sont alimentees exclusivement avec la liste de usernames valides definie dans la spec.
...
4. Mise en visibilite de signaux de vigilance qui pourraient passer inaperus a la simple lecture du tableau.
5. Reduction des oublis de partage du bon document.
6. Meilleure discipline collective sur les rituels de pilotage.
...
2. La note est creee sur la base du template `25` defini par l'equipe.
3. La note creee est rangee comme fille de la note `6`.
4. Le titre de la note contient la date de la reunion.
5. Le rapport d'avancement est insere sous forme tabulaire dans la note a partir des taches du projet `3`.
6. L'IA propose dans la note un ordre du jour suggere et des points de vigilance fondes sur l'analyse des taches.
7. Les suggestions restent presentees comme des propositions a arbitrer par les participants.
8. L'invitation agenda hebdomadaire existe et conserve le creneau du jeudi a `10:00`.
9. La description de l'invitation contient le lien vers la note de la semaine.
10. Le support obtenu est lisible en amont puis exploitable pendant la reunion pour la prise de notes.
11. L'invitation contient bien les participants `claire`, `etienne`, `louis`, `nadia`, `antoine` et `emma`.
