...
- **WF02**: canonical JSON + split specs; **native refactor (2026-04-27)** plus **didactic slice (2026-05-07)** — HTML node for task description, Manual `create_task` mapping, shortened coalesce/expressions (UTIL unwrap retained); **deferred** Data Table `createIfNotExists` (before first use, not on trigger); two Code nodes: `Ensure Merge Processed Input` + `Merge Decision`; unwrap via [subworkflow-dependencies.json](../workflows/wf02-document-validation/subworkflow-dependencies.json).
