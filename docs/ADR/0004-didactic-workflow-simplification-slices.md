...
2. **Practices (normative checklist for each slice):**
   ...
   - **MCP Client tool arguments** — Do not pass **empty optional fields** that break some servers; for **Manual** input, persist the resource-mapper **`schema`** with **`removed: true`** on unused tool parameters (or use **JSON** mode with a **minimal object** only if Manual reintroduces ghost fields—document the choice in the workflow spec).
