...
3. **JSON-only publish:** Importing **`workflow.json`** via the n8n UI or REST API does **not** go through MCP `validate_workflow`; gate (1) remains mandatory. MCP codegen is optional for that path. For automated JSON-only publish from this repository, use [tools/push-workflow-to-n8n-api.mjs](../tools/push-workflow-to-n8n-api.mjs) (`npm run deploy:workflow`), which runs local validation before `PUT` unless explicitly skipped. Portfolios may declare **`subworkflow-dependencies.json`** next to `workflow.json`; the REST deploy script then **PUT**s declared dependencies in order, injects remote **`workflowId`** values into the parent **in memory** (from `.env`, optional dependency root `id`, or matching parent **Execute Workflow** nodes), then **PUT**s the parent (see [DEVELOPMENT.md](DEVELOPMENT.md#portfolio-deploy-dependencies-manifest)).
