...
This repository is primarily an n8n/eXo workflow artifact workspace. It contains canonical `workflow.json` files per workflow, split specifications under `workflows/`, an optional [audit](audit-code-vs-natif.md), generated [inventory](inventory-code-nodes.json), and a minimal [tools/](../tools/) directory.
...
Example `config.env.example` files live **next to each workflow** (for example [workflows/wf02-validation-documentaire/config.env.example](../workflows/wf02-validation-documentaire/config.env.example)).
...
- **WF02**: import [workflows/wf02-validation-documentaire/workflow.json](../workflows/wf02-validation-documentaire/workflow.json); set variables from [config.env.example](../workflows/wf02-validation-documentaire/config.env.example); test webhook. See [wf02 README](../workflows/wf02-validation-documentaire/README.md).
- **WF03**: use [workflows/wf03-copil-hebdomadaire/README.md](../workflows/wf03-copil-hebdomadaire/README.md) and technical specs for IDs, notes, and agenda. Canonical JSON: [workflows/wf03-copil-hebdomadaire/workflow.json](../workflows/wf03-copil-hebdomadaire/workflow.json); raw API snapshot: [api-response.snapshot.json](../workflows/wf03-copil-hebdomadaire/fixtures/api-response.snapshot.json).
