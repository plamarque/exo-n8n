...
- **MCP OAuth2 fallback (`mcpOAuth2Api`):** after merge, the script loads `GET /api/v1/credentials` and resolves a binding in this order: `N8N_MCP_OAUTH2_CREDENTIAL_ID` (optional `N8N_MCP_OAUTH2_CREDENTIAL_NAME` is **only** the display label stored beside that id — not a lookup key) → `N8N_MCP_OAUTH2_RESOLVE_BY_NAME` (exact match on the credential **display name** in n8n for type `mcpOAuth2Api`) → if the instance has **exactly one** `mcpOAuth2Api` credential, use it. If **`N8N_MCP_OAUTH2_CREDENTIAL_ID`** or **`N8N_MCP_OAUTH2_RESOLVE_BY_NAME`** resolves successfully, deploy **applies** that `{id,name}` to **every** MCP Client node that uses OAuth2 authentication (overwrites existing references — use this to fix outdated ids in git after recreating a credential on the server). If only the **singleton** heuristic applies, deploy **only attaches** `mcpOAuth2Api` where the reference is still missing (same as before). Warnings are printed when resolve-by-name matches zero or more than one credential, or when multiple `mcpOAuth2Api` credentials exist and no env disambiguator is set.
- **OpenAI fallback:** if merge leaves `lmChatOpenAi` nodes without a reference, the script uses `N8N_OPENAI_CREDENTIAL_ID` / `N8N_OPENAI_CREDENTIAL_NAME` when set, otherwise a **single** `openAiApi` credential on the instance (with a warning when multiple exist). If the OpenAI credential list is ambiguous or unavailable, the script can reuse the `openAiApi` `{id,name}` from the first configured `lmChatOpenAi` on another workflow: set `N8N_OPENAI_REFERENCE_WORKFLOW_ID`, or rely on `N8N_WORKFLOW_ID_WF01` when set (typical: same binding as WF01 without editing WF03 in the n8n UI). Prefer fixing node ids to match the remote or attaching credentials once in the n8n UI over relying on env overrides when you need a different credential per workflow.
