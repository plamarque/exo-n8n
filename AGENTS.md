...
1. Read the relevant governance docs before changing workflows, scripts, configuration, or project documentation. Before **publishing** workflows to n8n, follow **`docs/WORKFLOW.md`** (*Deployment validation policy*): mandatory local validation on `workflow.json`. **Deploy:** either push canonical JSON via **`./tools/deploy.sh`** / **`npm run deploy:workflow`** (REST; see [`.env.example`](.env.example) and [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md#rest-deploy-to-n8n)), or deploy SDK `code` through the n8n MCP — in the MCP case, also run MCP `validate_workflow` on that `code` before `update_workflow` / `create_workflow_from_code` (recommended second gate). Do not mix channels unintentionally for the same publish step.
